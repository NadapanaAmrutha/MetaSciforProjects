...
#         'form': form,
#         'comments': comments,
#         'is_liked': is_liked  # Add the like status to the context
#     }
...
def like(request, post_id):
    ...
    # Return the updated likes count and whether the post is liked
    return JsonResponse({'likes': current_likes, 'liked': not liked})
...
def like(request, post_id):
    ...
    if not liked:
        Likes.objects.create(user=user, post=post)
        post.likes += 1
        liked = True  # The post is now liked
    else:
        # If the user has liked the post, remove the like
        Likes.objects.filter(user=user, post=post).delete()
        post.likes -= 1
        liked = False  # The post is now unliked
...
def comment(request, post_id):
    # Redirect to the PostDetail page for the specific post
     return HttpResponseRedirect(reverse('post-details', args=[post_id]))